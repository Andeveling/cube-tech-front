...
export class EverestModelO implements IModelO {
  ...
  public setFrameCost(window: WindowPVC, model: WindowModel) {
    ...
    this.windowCost.frameCost +=
      window.widthMeters * 2 * model.profile_frame.reinforcement_profile.price;
    // Heights PVC + Reinforcement
    this.windowCost.frameCost +=
      window.heightMeters * model.profile_frame.price;
    this.windowCost.frameCost +=
      window.heightMeters * model.profile_frame.reinforcement_profile.price;
  ...
  public setGlassCost(window: WindowModule) {
    ...
    if (window.glazingBeadAndGlass && window.glazingBeadAndGlass.glass) {
      this.windowCost.glassCost =
        window.glassArea * window.glazingBeadAndGlass.glass.price;
  ...
  public setGlazingBeadCost(window: WindowPVC) {
    if (
      ...
      this.windowCost.glazingBeadCost =
        window.glazingBeadLength * window.glazingBeadProfile.price;
    }
  }
}
