...
import { IModelZTD } from '../IModelZTD.interface';

export class EverestModelZTD implements IModelZTD {
  window: WindowPVC;
  ...
  public setGlazingBeadCost(window: WindowPVC) {
    if (
      this.window.glazingBeadAndGlass &&
      this.window.glazingBeadAndGlass.glazingBead &&
      this.window.glazingBeadProfile
    ) {
