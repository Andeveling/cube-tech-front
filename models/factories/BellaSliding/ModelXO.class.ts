...
export class BellaSlidingModelXO implements IModelXO {
  window: WindowPVC;
  ...
  public setVerticalTransom(baseWindow: WindowPVC, model: WindowModel) {
    if (model.profile_sash) {
      ...
      this.windowCost.transomCost +=
        baseWindow.heightMeters * model.profile_sash.price;
      this.windowCost.transomCost +=
    ...
    const widthFixed =
      baseWindow.width / 2 - model.profile_frame.cuttingSashSize;
  ...
  public setSash(windowModule: WindowModule, windowModel: WindowModel): void {
    const widthSash =
      windowModule.width / 2 - windowModel.profile_frame.cuttingSashSize;
    const heightSash =
        ...
        modules.forEach((sash) => {
          ...
          if (window.glazingBeadAndGlass && window.glazingBeadAndGlass.glass) {
            this.windowCost.glassCost +=
              sash.glassArea * window.glazingBeadAndGlass.glass.price;
          }
        });
      }
    }
