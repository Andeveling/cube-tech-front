...
export class BellaSlidingModelOXXO implements IModelOXXO {
  ...
  public setVerticalTransom(baseWindow: WindowPVC, model: WindowModel) {
    ...
    const fixedModuleRight = new WindowModule(widthFixed, heightFixed);

    this.window.addFixedModule(fixedModuleLeft);
    this.window.addFixedModule(fixedModuleRight);

    if (model.profile_frame && model.profile_sash) {
      fixedModuleLeft.setLeftProfile(model.profile_frame);
      fixedModuleLeft.setUpProfile(model.profile_frame);
      fixedModuleLeft.setDownProfile(model.profile_frame);
      fixedModuleLeft.setRightProfile(model.profile_sash);

      fixedModuleRight.setLeftProfile(model.profile_sash);
      fixedModuleRight.setUpProfile(model.profile_frame);
      fixedModuleRight.setDownProfile(model.profile_frame);
      fixedModuleRight.setRightProfile(model.profile_frame);
    }
  }
  // Sash
  public setSash(windowModule: WindowModule, windowModel: WindowModel): void {
    ...
    this.window.addSashModule(sashModuleTwo);
    if (windowModel.profile_sash) {
      sashModuleOne.setAllProfiles(windowModel.profile_sash);
      sashModuleOne.setGlazingBeadLength();
      sashModuleTwo.setAllProfiles(windowModel.profile_sash);
      sashModuleTwo.setGlazingBeadLength();
    }
    this.setSashCost(sashModuleOne, windowModel);
    this.setSashCost(sashModuleTwo, windowModel);
  }
